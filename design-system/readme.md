...
## Foundation
- Base palette: **near-black grounds** (faint green undertone) + **graphite neutrals**.
- **Twin leads, equal billing:** **deep desaturated forest/pine green** (Warren) and **deep garnet / oxblood** (a darkened dusty red — Carson) carry the brand together — in equal measure, often blended as a green⟷garnet **duo** (gradients, frames, the names). These are darkened, antique jewel tones — deliberately *not* bright sage/pink.
- **Antique gold** is demoted to a **fine metallic tie**: the "&" between the names, divider centers, corner ornaments, small diamonds, focus/hover glints.
- Type: **Cinzel** (Deco Roman caps, titling) · **Cormorant Garamond** (warm serif body) · **Poiret One** (geometric Deco labels) · **Cinzel Decorative** (rare flourishes).
...
**Punctuation & ornament.** A diamond (`✦` / rotated square) or the divider motif separates ideas rather than exclamation points. Ampersands are a brand signature — set in Cinzel Decorative and **gold** between the two names ("Warren **&** Carson"), the metallic tie joining the two grooms' colors.
...
- Names: **Warren & Carson** (green = Warren, rose = Carson; the "&" set in gold)

---

## VISUAL FOUNDATIONS

**Color & vibe.** Dark, candle-lit, regal. Pages sit on `--ink-800` (#12150F) — near-black with a faint green undertone so it never reads as pure tech-black. Text is warm parchment (`--parchment-100`, #F4EFE3). **Deep pine green and deep garnet/oxblood are the twin leads, in equal measure** — the two grooms. They are darkened antique jewel tones (green ~#3E4F3A–#56684F, garnet ~#743D35–#93534A), chosen to read rich and regal and to avoid any bright sage+pink association. They appear solo, mirrored (alternating per section/item), and blended as a green⟷garnet **duo** (`--duo-sheen` for the names and primary CTA, `--duo-line` for frames and rules). **Gold is a fine metallic tie only** — the "&", divider centers, corner filigree, small diamonds, hover/focus glints — never a dominant fill. Avoid: bright/emerald greens, pink roses, bluish-purple gradients, neon, pure white, pastel, large gold fills.

**Type.** High contrast between geometric Deco caps (Cinzel) and a warm humanist serif (Cormorant). Display tracking is generous (0.04em); eyebrows are very wide (0.34em). Italic Cormorant carries emphasis and warmth. No script/cursive faces.

**Backgrounds.** Solid dark grounds, occasionally layered with: a **rising-sun halo** behind hero titling; a **faint Celtic knot lattice** texture (`assets/pattern-tile.svg`, a seamless over-under diagonal plait) at ~4% opacity over full pages. No photographic full-bleeds by default (photos live in framed slots); no loud gradients — the only gradients are the green→garnet duo on names/CTAs and the metallic `--gilt-sheen` on small gold elements.

**Linework & framing (the signature).** Thin **neutral parchment** hairlines (`--line`, ~16% parchment) for quiet structure; **green→garnet duo borders** (`--duo-line`, via a padding-box/border-box gradient) for important containers like `Frame`; per-groom rules (`--line-green`, `--line-rose`) when a single color is wanted. **Interlaced Celtic-knot corner ornaments** (a small gold tie) sit at each corner of `Frame`. Dividers run a green→garnet gradient (green left, garnet right) with a centered gold trinity knot. The motif is knotwork throughout — never botanical filigree.
...
**Borders.** `--line` (neutral parchment hairline), `--line-strong`, `--line-soft` for quiet structure; `--line-green` / `--line-rose` for single-groom rules; `--line-gold` for the occasional metal hairline. Important frames use the `--duo-line` green⟷rose gradient border.

**Shadows & elevation.** Deep, soft, near-black on dark grounds: `--shadow-sm/md/lg`. For emphasis/focus, a warm **gilt glow** (`--glow-gold`) — a small gold ring + amber halo — is the metallic accent; green and rose buttons carry their own colored halo on hover.

**Transparency & blur.** The sticky nav uses `rgba(12,14,12,0.82)` + `backdrop-filter: blur(10px)`. Soft tinted fills for badges (`rgba(gold/green/rose, ~0.12–0.18)`). Used sparingly.
...
**Hover / press states.** Hover: the duo button brightens + gains the gilt glow; green/rose buttons brighten with a matching colored halo; outline/ghost shift toward `--accent-strong` (green) and pick up a hairline; cards lift 2px with a stronger rule. Press: a 1px downward nudge (`translateY(1px)`). Focus: a green halo (`--focus-ring`).

**Cards.** Two flavors — **dark** (`--surface-card` #242922 with a gold hairline + `--shadow-md`) and **parchment** (warm cream with a gold border, for contrast moments like the venue). Interactive cards lift on hover. Near-sharp corners.
...
- **No emoji. No icon font.** If a functional UI ever needs line icons (e.g. a richer RSVP flow), use a **thin-stroke, 1.5px set** — **Lucide** is the recommended match, tinted green/rose/parchment to suit. *(Not yet in use — added only if needed.)*
- Logos/marks: the `monogram.svg` Celtic knot name-plate is the primary mark; the "W & C" Cinzel wordmark (gold Cinzel Decorative ampersand) is the secondary lockup.
